...
sealed class AutoBackgroundFindNextDividendAnnoucements : AutoBackgroundFindNextAnnoucements
{
	public AutoBackgroundFindNextDividendAnnoucements(
			IServiceProvider serviceProvider, ILogger<AutoBackgroundFindNextDividendAnnoucements> logger
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
			...
			try
			{
				...
				var checkpoint = await GetCheckpoint(
					...
					checkpointType: CheckpointEntity.CHECKPOINT_INCOMING_DIVIDEND,
					cancellationToken
				);
				...
				{
					Logger.LogInformation("Finding next dividend/distribution announcements for market {market}...", country);
					var finhubClient = scope.ServiceProvider.GetRequiredService<IFinHubClient>();
					var incomingEvents = await finhubClient.GetIncomingDividendAnnouncementsAsync(country, cancellationToken: cancellationToken);
					if (incomingEvents.Status != 200)
					{
						Logger.LogError("Failed to fetch incoming dividend/distribution announcements for market {market}. Status: {status}, Message: {message}", country, incomingEvents.Status, incomingEvents.Message);
					}
					else
					{
						Logger.LogInformation("Incoming dividend/distribution announcements for market {market}: {event}", country, JsonSerializer.Serialize(incomingEvents.Data));
