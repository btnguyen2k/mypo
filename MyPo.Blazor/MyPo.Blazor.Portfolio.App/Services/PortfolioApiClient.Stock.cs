...
public partial class PortfolioApiClient
{
	...
	{
		var endpoint = IPortfolioApiClient.API_STOCKS_SYMBOL_OVERVIEW
	...
	{
		var endpoint = IPortfolioApiClient.API_STOCKS_SYMBOL_INFO
			.Replace("{symbol}", symbol, StringComparison.OrdinalIgnoreCase);
		using var httpResult = await BuildAndSendRequestAsync(
			requestHttpClient,
			HttpMethod.Get, baseUrl, endpoint,
	...
	{
		var endpoint = IPortfolioApiClient.API_STOCKS_SYMBOL_QUOTE_AT_DATE
			.Replace("{symbol}", symbol, StringComparison.OrdinalIgnoreCase)
