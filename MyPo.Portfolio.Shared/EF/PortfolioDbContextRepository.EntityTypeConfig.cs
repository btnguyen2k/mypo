...
sealed class PortfolioRecEntityTypeConfiguration : GenericEntityTypeConfiguration<PortfolioRec, string>
{
	public override void Configure(EntityTypeBuilder<PortfolioRec> builder)
	{
		...
		builder.Property(builder => builder.ParentId).HasColumnName("parent_id").HasMaxLength(48);
		...
		builder.Property(builder => builder.ConcurrencyStamp).HasColumnName("concurrency_stamp").IsConcurrencyToken();
	}
}
